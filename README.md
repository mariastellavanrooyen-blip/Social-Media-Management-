# Marisol CRM — Bulk Cold Email Tool

Lightweight CRM and bulk cold-email tool for a single user (Maria Van Rooyen Social Studio).
Python/FastAPI backend, plain HTML/JS frontend (no build step), installable as a PWA. SQLite
locally by default; Postgres (e.g. a free Neon or Supabase database) in production, via
`DATABASE_URL`.
...
- ✅ Rebrand: cream/charcoal/terracotta editorial visual identity (Playfair Display + Inter)
- ✅ Installable PWA: manifest, homescreen icons, offline app-shell caching (API calls always
  hit the network live — see "PWA" below)
...
backend/
  ...
  scripts/
    ...
    gen_icons.py          Regenerates the PWA homescreen icons (needs Pillow)
...
frontend/
  ...
  manifest.json      PWA manifest (name, icons, theme/background color, standalone display)
  sw.js              Service worker: caches the app shell for offline load, never intercepts
                     /api/* — client/campaign data always comes from the network
  static/            CSS + vanilla JS
    pwa.js           Registers sw.js, included on every page
    icons/           Generated "MV" monogram icons (192/512/apple-touch)
...
The site is installable — on Android Chrome, look for "Add to Home screen" / an install
prompt; on iOS Safari, Share → "Add to Home Screen". It opens as a standalone app window
using the "MV" monogram icon (`frontend/static/icons/`), generated by `scripts/gen_icons.py`
(requires Pillow — not a runtime dependency, so it's not in `requirements.txt`) — rerun that
if you want the icon design changed.

`sw.js` caches the app shell (HTML/CSS/JS/icons/manifest) so the app still opens with no
connection, but it deliberately never caches anything under `/api/` — client records, upload
state, and send status always come straight from the network. That's a deliberate limit, not
an oversight: showing stale client data while offline would be worse than showing nothing, and
none of this app's actual work (editing clients, sending mail) can happen offline anyway.
